...
public enum DnsProvider {
    ...
    DnsProvider(int id, String label, List<String> ipv4, List<String> ipv6) {
        this.id = id;
        this.label = label;
        this.ipv4 = ipv4;
        this.ipv6 = ipv6;
    ...
    /**
     * Поиск провайдера по списку IP адресов из конфига
     */
    public static Optional<DnsProvider> findByAddresses(List<String> addresses) {
        return Arrays.stream(values())
                .filter(p -> {
                    List<String> providerAddrs = new ArrayList<>();
                    providerAddrs.addAll(p.ipv4);
                    providerAddrs.addAll(p.ipv6);
                    return addresses.containsAll(providerAddrs) || providerAddrs.containsAll(addresses);
                })
                .findFirst();
    }

    public static Optional<DnsProvider> findById(int id) {
        return Arrays.stream(values())
                .filter(p -> p.id == id)
                .findFirst();
    }
